...
# This creates trackbars to adjust various values.
...
## Loading torch model
model = Model()
model.load_state_dict(torch.load(model_path, map_location=device))  # Load model to GPU/CPU
...
lineType = 4
## Stores previously drawn circles to give continuous lines and also store current color and size of pen
circles = []
...
while True:
    ...
    if not results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            model_input = torch.tensor(landmark_list, dtype=torch.float).unsqueeze(0).to(device)  # Input to GPU
            with torch.no_grad():
            ...
            if action == 'Erase':
                ...
                circles = [circle for circle in circles if not is_position_out_of_bounds(circle[0], top_left, bottom_right)]

    ## Draw circles
    for circle in circles:
        cv2.circle(frame, circle[0], circle[2], circle[1], -1)
